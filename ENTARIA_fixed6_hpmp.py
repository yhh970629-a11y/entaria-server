...
# HP 포션 설정
HP_POTION_DEFAULT_KEY = "end"
HP_POTION_DEFAULT_THRESHOLD = 50
...
def detect_hp_percent():
    """게임 클라이언트 좌측 하단의 빨간 HP 게이지를 감지합니다."""
    ...
    try:
        # MP와 동일하게 게임 화면 좌측 하단 HUD 영역 탐색
        roi_x = 0
        roi_y = int(gh * 0.84)
        ...
        filled_width = float(
            np.median(stable_widths)
        )

        # MP와 동일한 방식으로 전체 게이지 폭 계산
        full_width = gw * 0.17

        full_width = max(
            120.0,
            min(220.0, full_width)
        )
...
def create_main_window():
    ...
    tk.Label(
        hp_card,
        text="좌측 하단의 빨간 HP 게이지를 감지해 설정한 비율 이하에서 포션을 사용합니다.",
        bg=CARD,
        fg=MUTED,
        font=(FONT, 8)
    ).pack(anchor="w", padx=18, pady=(0, 14))
    ...
    hp_key_var = tk.StringVar(
        value=HP_POTION_DEFAULT_KEY
    )

    hp_key_entry = tk.Entry(
        hp_setting_row,
        textvariable=hp_key_var,
        width=8,
        bg=CARD_3,
        fg=TEXT,
        insertbackground=TEXT,
    ...
    hp_threshold_var = tk.StringVar(
        value=str(HP_POTION_DEFAULT_THRESHOLD)
    )
    ...
    hp_threshold_spin = tk.Spinbox(
        ...
        to=95,
        increment=5,
        textvariable=hp_threshold_var,
        width=5,
        bg=CARD_3,
        fg=TEXT,
        buttonbackground=CARD_3,
        insertbackground=TEXT,
        relief="flat",
        font=(FONT, 10, "bold")
    )
    hp_threshold_spin.pack(
        side="left",
        padx=(8, 4),
        ipady=3
    )
        ...
        try:
            while (
                ...
                if hp_percent is None:
                    hp_status_var.set("HP 게이지 감지 중")
                    time.sleep(HP_POTION_CHECK_INTERVAL)
                    continue

                try:
                    key, threshold = get_hp_potion_settings()
                except ValueError:
                    hp_status_var.set("설정 확인 필요")
                    time.sleep(HP_POTION_CHECK_INTERVAL)
                    continue

                if (
                    hp_percent <= threshold
                    and time.monotonic() - last_potion_time >= 0.8
                ):
                    try:
                        pyautogui.press(key)
                        ...
                        hp_status_var.set(
                            f"포션 사용 • HP 약 {hp_percent:.0f}%"
                        )

                    except Exception as e:
                        log(f"HP 포션 키 입력 오류: {e}")
                        hp_status_var.set("키 입력 오류")

                else:
                    hp_status_var.set(
                        f"감지 중 • HP 약 {hp_percent:.0f}%"
                    )
    ...
    def start_hp_potion():
        ...
        hp_potion_thread = threading.Thread(
